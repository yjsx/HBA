...
void localba(LAYER& layer, int gap_index){
    ...
    {
      x_buf[j].R = layer.pose_vec[gap_index*GAP+j].q.toRotationMatrix();
      x_buf[j].p = layer.pose_vec[gap_index*GAP+j].t;
    }
    size_t mem_cost = 0;
    if(layer_num != 1)
        for(int j = gap_index*GAP; j < gap_index*GAP+WIN_SIZE; j++)
            src_pc[j-gap_index*GAP] = (*layer.pcds[j]).makeShared();

    for(int j = gap_index*GAP; j < gap_index*GAP+WIN_SIZE; j++){
        pcl::PointCloud<PointType>::Ptr pc(new pcl::PointCloud<PointType>);
        mypcl::loadPCD(layer.data_path, pcd_name_fill_num, pc, j, "pcd/");
        raw_pc[j-gap_index*GAP] = pc;
        src_pc[j-gap_index*GAP] = (*raw_pc[j-gap_index*GAP]).makeShared();
    }
    ...
    // if(abs(residual_pre-residual_cur)/abs(residual_cur) < 0.05){
    //     for(int j = 0; j < WIN_SIZE*(WIN_SIZE-1)/2; j++)
    //         layer.hessians[gap_index*(WIN_SIZE-1)*WIN_SIZE/2+j] = hess_vec[j];
    //     break;
    // }
    std::cout<<gap_index<<" residual_cur:"<<residual_cur<<std::endl;
    pcl::PointCloud<pcl::PointXYZ>::Ptr color_cloud(new pcl::PointCloud<PointType>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr tmpcloud(new pcl::PointCloud<PointType>);

    sensor_msgs::PointCloud2 localba_msg;

    for(int j = 0; j < WIN_SIZE; j++){;
        layer.pose_vec[gap_index*GAP+j].q = Quaterniond(x_buf[j].R);
        layer.pose_vec[gap_index*GAP+j].t = x_buf[j].p;

        mypcl::transform_pointcloud(*raw_pc[j], *tmpcloud, x_buf[j].p, Quaterniond(x_buf[j].R));
        *color_cloud += *tmpcloud;
    }
    pcl::toROSMsg(*color_cloud, localba_msg);
...
int main(int argc, char** argv)
{
    ...
    // for(int i =0; i< hba.layers[0].part_length;i++)
    //     for(int loop = 0; loop < 3; loop ++)
    //         localba(hba.layers[0], i);


    for(int loop = 0; loop < 10; loop ++)
        localba(hba.layers[0], 3);
