...
std::vector<std::queue<sensor_msgs::PointCloud2ConstPtr>> cloud_buf_list;
std::vector<std::queue<nav_msgs::Odometry>> pose_buf_list;
std::mutex mutex_lock;

void CloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg, std::queue<sensor_msgs::PointCloud2ConstPtr> * buf){
    mutex_lock.lock();
    buf->push(laserCloudMsg);
    mutex_lock.unlock();
}
void PoseHandler(const nav_msgs::OdometryConstPtr &poseMsg, std::queue<nav_msgs::OdometryConstPtr> * buf){
    mutex_lock.lock();
    buf->push(poseMsg);
    mutex_lock.unlock();
}

void synchronize(std::string topic, std::queue<sensor_msgs::PointCloud2ConstPtr> cloud_buf,
							std::queue<nav_msgs::OdometryConstPtr> pose_buf){
    const float TIMESTAMP_TOLERANCE = 0.001; 

    while (1) {
		if(!cloud_buf.empty() && !pose_buf.empty()){
			auto& cloud_front = cloud_buf.front();
			auto& pose_front = pose_buf.front();
			
			float timestamp_diff = std::abs(cloud_front->header.stamp.toSec() - pose_front->header.stamp.toSec());

			if (timestamp_diff <= TIMESTAMP_TOLERANCE) {
				cloud_buf.pop();
				pose_buf.pop();
				std::cout<<topic<<" received!"<<std::endl;
			} else if (cloud_front->header.stamp.toSec() < pose_front->header.stamp.toSec()) {
				cloud_buf.pop();
			} else {
				pose_buf.pop();
			}
...
int main(int argc, char** argv)
{
	...
	std::vector<ros::Subscriber> cloud_sub_list(10);
	std::vector<ros::Subscriber> pose_sub_list(10);
	std::vector<std::string> topic_list(10);
	cloud_buf_list.resize(10);
	pose_buf_list.resize(10);
	...
	std::vector<Submap> robots = get_submap_info(data_path+"config.yaml");

	sensor_msgs::PointCloud2 debugMsg, cloudMsg, outMsg;
	vector<mypcl::pose> pose_vec;
	for(int i = 0; i < robots.size(); i++) {
        cloud_sub_list[i] = nh.subscribe<sensor_msgs::PointCloud2>(robots[i].topic+"/cloud", 100, boost::bind(&CloudHandler,_1, &cloud_buf_list[i]));
        pose_sub_list[i] = nh.subscribe<sensor_msgs::PointCloud2>( robots[i].topic+"/pose", 100, boost::bind(&PoseHandler,_1, &pose_buf_list[i]));
		auto bound_synchronize = std::bind(synchronize, robots[i].topic, cloud_buf_list[i], pose_buf_list[i]);
		std::thread synchronize_thread(bound_synchronize);
    }
